...
3. Seite schließen, erneut öffnen und prüfen, dass `Bordeaux 47` noch vorhanden ist.
4. `Löschen` wählen, Seite neu laden und prüfen, dass die Notiz ausschließlich im
   30-Tage-Papierkorb erscheint.
5. `Wiederherstellen` wählen, neu laden und prüfen, dass die Notiz wieder aktiv ist.
6. Unter `Sicherung und Löschung` eine Sicherung exportieren.
7. Eine sichtbar veränderte oder fremde JSON-Datei darf nicht importiert werden.
8. Die vollständige Löschung erst mit der angezeigten exakten Bestätigungsphrase freigeben.
9. Seite neu laden und prüfen, dass keine Notiz wieder erscheint.
10. Die Schritte 1–5 auf dem Handy wiederholen und Hoch-/Querformat prüfen.
